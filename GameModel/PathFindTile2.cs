...
namespace GameModel
{
    public class PathFindTile2 : IHasNeighbours2<PathFindTile2>
    {
        ...
        public List<PathFindTile2> Neighbours { get; set; }
        public Dictionary<Hex, double> MoveCost { get; set; }
        public bool HasCumulativeCost { get; set; }

        public PathFindTile2(Hex hex, List<Edge> edges)
        {
            Hex = hex;
            Edges = edges;
        ...
        public void LoadNeighbours(HashSet<PathFindTile2> loadedPathFindTiles, List<Edge> neighbourEdges, bool usesRoads, bool isBeingTransportedByWater, Dictionary<EdgeType, int> edgeMovementCosts, Dictionary<TerrainType, int> terrainMovementCosts, TerrainType canStopOn)
        {
            Neighbours = new List<PathFindTile2>();
            ...
            foreach (var edge in neighbourEdges)
            {
                ...
                if (moveCost < Terrain.Impassable)
                {
                    ...
                    {
                        neighbourPathFindTile = new PathFindTile2(edge.Destination.Hex, edge.Destination.Neighbours);
